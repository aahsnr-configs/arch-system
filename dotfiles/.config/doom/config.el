...
(after! eglot
 ;; Use basedpyright as the LSP server for Python
 ;; Basedpyright is a Pyright fork focused on type checking, type inference,
 ;; and module resolution - core language server features
 (set-eglot-client! '(python-mode python-ts-mode)
                    '("basedpyright-langserver" "--stdio"))

 ;;; Delegate capabilities to specialized tools
 ;; These capabilities are better handled by dedicated Emacs packages:
 (setq eglot-ignored-server-capabilities
       '(:hoverProvider                      ; â†’ eldoc + eldoc-box handles documentation
         :documentFormattingProvider         ; â†’ apheleia + ruff handles formatting
         :documentRangeFormattingProvider    ; â†’ apheleia + ruff handles range formatting
         :documentOnTypeFormattingProvider   ; â†’ apheleia + ruff handles on-type formatting
         :foldingRangeProvider))             ; â†’ tree-sitter handles code folding

 ;;; Keep enabled (handled by eglot):
 ;; - :documentHighlightProvider (highlights symbol occurrences)
 ;; - :completionProvider (code completion)
 ;; - :definitionProvider (go-to-definition)
 ;; - :referencesProvider (find references)
 ;; - :renameProvider (rename symbols)
 ;; - :codeActionProvider (code actions/quickfixes)
 ;; - :signatureHelpProvider (function signatures)

 ;;; Basedpyright Configuration: Focus on TYPE CHECKING ONLY
 ;; Division of labor:
 ;; BASEDPYRIGHT handles: Types, module resolution, type stubs, type inference
 ;; RUFF handles: Linting, code style, unused code, syntax errors, import sorting
               ...
               '(:basedpyright
                 (:analysis
                  (:typeCheckingMode "recommended"  ; Balance between strict and permissive
                   ...
                   (;; Unused code detection â†’ Ruff handles via F401, F841 rules
 ...
 ;; This ensures diagnostics from both eglot (basedpyright) and flymake (ruff)
 ;; are shown through eldoc-box at point, avoiding duplicate popups
...
;; - Code style enforcement (PEP 8 via pycodestyle rules)
;; - Unused imports/variables (F401, F841, etc.)
;; - Syntax error detection
;; - Import organization and sorting (isort-equivalent)
;; - Code complexity checks (McCabe, etc.)
;; - Common bug patterns (flake8-bugbear equivalent)
;; - Security issues (bandit-equivalent)
;; - 800+ built-in rules from Flake8 ecosystem
(use-package! flymake-ruff
  :after flymake
  :hook (eglot-managed-mode . flymake-ruff-load))

;; Configure flymake to work harmoniously with eldoc-box
;; - Diagnostics shown via eldoc-box at point (not inline at end of line)
;; - Fringe indicators for quick visual feedback
;; - Fast responsiveness for immediate feedback
(after! flymake
