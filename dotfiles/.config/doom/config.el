...
(after! tex
  ;; Set LuaLaTeX as the default engine (modern, Unicode-aware)
  ;; Options: 'luatex (LuaLaTeX), 'xetex (XeLaTeX), 'default (pdfLaTeX)
  (setq TeX-engine 'luatex)

  ;; Use build directory for cleaner project organization
  (setq TeX-output-dir "build/")

  ;; Enable shell-escape for minted, gnuplot, and other external programs
  (setq LaTeX-command "latex -shell-escape")

  ;; Auto-save before compiling
  (setq TeX-save-query nil)

  ;; Don't ask for confirmation when cleaning auxiliary files
  (setq TeX-clean-confirm nil))

(after! latex
  ;; Add latexmk commands for each engine
  ;; latexmk automatically handles multiple passes for references, bibliography, etc.
  (add-to-list 'TeX-command-list
               '("LaTeXmk" "latexmk -pdf -pdflatex='pdflatex -shell-escape -interaction=nonstopmode' -f -outdir=%o %S"
                 TeX-run-TeX nil t
                 :help "Run latexmk with pdfLaTeX") t)
  ...
  (add-to-list 'TeX-command-list
               '("XeLaTeXmk" "latexmk -pdfxe -pdfxelatex='xelatex -shell-escape -interaction=nonstopmode' -f -outdir=%o %S"
                 TeX-run-TeX nil t
  ...
  ;; Set default command based on engine
...
(after! citar
  ;; Doom's biblio module already provides citar with vertico
  ;; We just need to configure paths and enable our project-aware system
