...
(after! org
  ...
  (setq org-format-latex-header
        "\\documentclass[varwidth=\\maxdimen, margin=3pt]{standalone}
\\usepackage{amsmath}
\\usepackage{amssymb}
\\usepackage{fontspec}
\\defaultfontfeatures{Scale=MatchLowercase}
\\usepackage{xcolor}
\\AtBeginDocument{\\color[HTML]{c0caf5}}
\\pagestyle{empty}
\\tolerance=1000")
  ...
  (add-to-list 'org-preview-latex-process-alist
               '(lualatex
                 ...
                 :description "lualatex > xdv > dvisvgm > svg"
                 :message "you need to install 'lualatex' and 'dvisvgm'."
                 :image-input-type "dvi"
                 :image-output-type "svg"
                 :image-size-adjust (1.0 . 1.0)
                 :latex-compiler ("lualatex --output-format=dvi -interaction nonstopmode -output-directory %o %f")
                 :image-converter ("dvisvgm --scale=1.5 --bbox=min --no-fonts --page=1- --optimize --relative --output=%O %f")))
