#+TITLE: My Doom Emacs Config (with Oglot Integration - CORRECTED)
...
** Oglot - with Jupyter Integration
Oglot enables Eglot (LSP client) to work seamlessly with Org mode source blocks,
including those overridden to use Jupyter for execution. This provides full IDE
features while maintaining Jupyter's interactive execution capabilities.

When ~org-babel-jupyter-override-src-block "python"~ is set, Python blocks are
executed by Jupyter but are STILL identified as "python" blocks for editing.
Therefore, we define ~org-babel-edit-prep:python~ (NOT jupyter-python).
#+begin_src emacs-lisp
(use-package! oglot
  :after (org eglot)
  :config
