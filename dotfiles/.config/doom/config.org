...
** Jupyter
*** Core Jupyter Setup
Doom's =+jupyter= flag handles loading =ob-jupyter=. We configure default header
arguments and ensure python blocks are overridden with jupyter-python for
seamless execution.
...
*** Corfu Completion Compatibility
Jupyter's built-in completion conflicts with Corfu. We remove it to prevent
"wrong-type-argument markerp nil" errors while preserving LSP-based completion.
...
*** LSP in Org Source Blocks (oglot-style)
This implementation allows full Eglot LSP support when editing source blocks
with =C-c '=. It requires:
1. A =:tangle= header pointing to a real file
2. The tangled file to exist (run =org-babel-tangle= first)
3. The block content to appear exactly once in the tangled file
...
The =:comments link= header argument adds markers in tangled files that help
identify the source of each block. This is useful for debugging and keeping
tangled files synchronized with the org file.
...
*** Helper Functions
Utility functions for working with Jupyter and literate programming.
...
*** ANSI Color Support
Apply ANSI color codes to babel results for colored terminal output.
